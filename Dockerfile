...
RUN certbot certonly --standalone --webroot -w /usr/share/nginx/html -d "$DOMAIN" --webroot -w /usr/share/nginx/api -d "$API_NAME" --webroot -w /usr/share/nginx/auth -d "$KEY_DOMAIN" -m bnakamur@hotmail.com --agree-tos --non-interactive