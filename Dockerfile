...
RUN certbot certonly --standalone -w /usr/share/nginx/html -d "$DOMAIN" -w /usr/share/nginx/api -d "$API_NAME" -w /usr/share/nginx/auth -d "$KEY_DOMAIN" -m bnakamur@hotmail.com --agree-tos --non-interactive -v